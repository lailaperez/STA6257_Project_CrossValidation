...
The steps for k-fold Cross Validation are as follows and were adapted from Song QC, Tang C, Wee S. Making Sense of Model Generalizability: A Tutorial on Cross-Validation in R and Shiny. Advances in Methods and Practices in Psychological Science. 2021;4(1). doi:10.1177/2515245920947067

1. Prepare the data.  
To do this, you take all the observations and randomly shuffle them.  Once it has been shuffled, if N = all of the observations and k is the number of folds you want to use, then you would assign the observations into folds that are roughly N/k large.  When placing the observations into the folds, you would do so without replacement so each observation would be in 1 and only 1 fold.  For example,  If you wanted k=10 folds and you had 200 observations, each fold would be 200/10 = 20 large.  If you were using 201 observations, then one of the folds would have 21, the rest would have 20.  

2. Split the folds into training sets and a test set.  
In the previous example, if you had 10 folds, we could choose the first set to be the test set and the other 9 would be the training set.  It doesn’t make a difference which one you choose as all of the folds will eventually be test folds against the other 9.

3. Fit model to the training data.
Take the model you are going to use for prediction and fit it to the training data.  Continuing the example, you would use the 9 training folds to fit the model.
Take the fitted model you developed in step 3 and apply it to the 1 test fold.  After applying it to the model, you would take the resulting prediction and determine the accuracy by comparing what was predicted from the training folds to the actual values from the test fold.

4. Repeat steps 2 - 4.  
In the example, if you were using k=10, then you would pick one of the folds you have not previously used and make it the test fold and the other 9 the training fold.  In this way, every observation will be a member of the test fold once and training folds 9 times.

5. Take accuracies
After each fold has been used as the test fold, take all the accuracies determined from step 4 and come up with an average accuracy for the data set.
...
3. Split the inner loop into training sets and validation sets.  In a 5 * 2 split, where 5 is the number of folds, the inner loop is broken into 2 training sets and 2 validation sets.
